...
function buildSafeRouteGeometry(route = {}) {
...
function normalizePoiRecord(record = {}, fallbackCategory = "") {
    const lat = Number(record?.lat ?? record?.latitude);
    const lon = Number(record?.lon ?? record?.lng ?? record?.longitude);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
    if (Math.abs(lat) > 90 || Math.abs(lon) > 180) return null;

    const category = String(record?.category || fallbackCategory || "").trim().toLowerCase();
    const resolvedCategory =
        category === "ev" || category === "fuel" || category === "police"
            ? category
    ...
    const fallbackName =
        resolvedCategory === "ev"
            ...
            : resolvedCategory === "police"
                ? "Police Station"
                : "Fuel Station";
    const parsedDistance = Number(record?.distanceToRouteMeters);
    ...
    return {
        ...
        dataStatus: String(record?.dataStatus || record?.availability || "Source: OpenStreetMap").trim(),
        distanceToRouteMeters: Number.isFinite(parsedDistance) ? Math.max(0, Math.round(parsedDistance)) : null
    };
}

function normalizePoiCollection(records = [], fallbackCategory = "") {
    return (Array.isArray(records) ? records : [])
        .map((record) => normalizePoiRecord(record, fallbackCategory))
        .filter(Boolean);
}

function normalizeTollPlazaCollection(records = []) {
    return (Array.isArray(records) ? records : [])
        .map((record, index) => {
            const lat = Number(record?.lat ?? record?.latitude);
            const lon = Number(record?.lon ?? record?.lng ?? record?.longitude);
            if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
            if (Math.abs(lat) > 90 || Math.abs(lon) > 180) return null;
            const distanceFromStartKm = Number(record?.distanceFromStartKm);
            const estimatedCost = Number(record?.estimatedCost);
            return {
                ...record,
                name: String(record?.name || `Toll Plaza ${index + 1}`).trim() || `Toll Plaza ${index + 1}`,
                lat,
                lon,
                distanceFromStartKm: Number.isFinite(distanceFromStartKm) ? Math.max(0, distanceFromStartKm) : 0,
...
async function geocode(place, options = {}) {
    ...
    const resolved = normalizeLatLngPair([Number(bestCandidate.lat), Number(bestCandidate.lon)]);
    if (!resolved) throw new Error(`Location "${inputPlace}" resolved to invalid coordinates`);
    return resolved;
}
...
async function findRoute(type = "default") {
    ...
    try {
        ...
        const srcCoords = normalizeLatLngPair(sourceResolved.coords);
        const destCoords = normalizeLatLngPair(destinationResolved.coords);
        if (!srcCoords || !destCoords) {
            throw new Error("Source or destination coordinates are invalid. Please reselect locations.");
        }
        ...
        for (const stop of viaStops) {
            const coords = normalizeLatLngPair(await geocode(stop, { referenceCoords }));
            if (!coords) throw new Error(`Via stop "${stop}" could not be resolved.`);
        ...
        selectedRouteGeometry = buildSafeRouteGeometry(selectedRoute);
        if (!selectedRouteGeometry) {
            throw new Error("Route geometry is invalid. Please try a different source/destination.");
        }
        selectedRouteCoordinates = selectedRouteGeometry.coordinates.map(([lon, lat]) => [lat, lon]);
        ...
        const weatherSnapshot = await getRouteWeatherSnapshot(selectedRouteGeometry.coordinates, hour);
        ...
        const nextSnapshot = {
            ...
            purpose,
            priority,
            routeCoordinates: selectedRouteGeometry.coordinates,
            selectedVariantKey
        };
        ...
        await refreshEvStations(selectedRouteGeometry.coordinates, prediction);
        ...
        refreshTrafficHotspots(selectedRouteGeometry.coordinates, prediction);
        refreshTollPlazas(selectedRouteGeometry.coordinates, distanceKm, vehicle, prediction);
...
function drawRouteOnMap(routeVariants, selectedVariantKey, prediction, displayMode = "all") {
    ...
    if (!map || !routeVariants?.fastest?.route) return;
    ...
    const fastestMins = Math.round(Number(routeVariants.fastest.route.duration || 0) / 60);
    ...
    variantsToRender.forEach((variant) => {
        const safeGeometry = buildSafeRouteGeometry(variant?.route || {});
        if (!safeGeometry) return;
        ...
        const delay = Math.max(0, Math.round(Number(variant.route.duration || 0) / 60) - fastestMins);
        const tooltipText = `${style.label} route | Traffic: ${prediction.congestion_level} | Delay: ${delay} mins`;

        const layer = L.geoJSON(safeGeometry, {
            style: {
                color: style.color,
...
function addRouteMarkers(srcCoords, destCoords, srcName, destName, viaCoords = [], viaNames = []) {
    ...
    const normalizedSource = normalizeLatLngPair(srcCoords);
    const normalizedDestination = normalizeLatLngPair(destCoords);
    if (!normalizedSource || !normalizedDestination) {
        throw new Error("Unable to place route markers. Please set valid source and destination.");
    }

    const normalizedViaStops = [];
    (Array.isArray(viaCoords) ? viaCoords : []).forEach((coords, index) => {
        const normalized = normalizeLatLngPair(coords);
        if (!normalized) return;
        normalizedViaStops.push({
            coords: normalized,
            name: viaNames[index] || `Stop ${normalizedViaStops.length + 1}`
        });
    });
    ...
    sourceMarker = L.marker(normalizedSource, { icon: startIcon }).addTo(map).bindPopup(`<strong>Start:</strong> ${srcName}`).bindTooltip(`Start: ${srcName}`, { direction: "top" });
    sourceMarker.on("mouseover", () => sourceMarker.openTooltip());
    normalizedViaStops.forEach((stop, index) => {
        const marker = L.marker(stop.coords, { icon: viaIcon(index) })
            .addTo(map)
            .bindPopup(`<strong>Via ${index + 1}:</strong> ${stop.name}`)
            .bindTooltip(`Via ${index + 1}: ${stop.name}`, { direction: "top" });
        marker.on("mouseover", () => marker.openTooltip());
        viaMarkers.push(marker);
    });
    destMarker = L.marker(normalizedDestination, { icon: endIcon }).addTo(map).bindPopup(`<strong>Destination:</strong> ${destName}`).bindTooltip(`Destination: ${destName}`, { direction: "top" });
...
function buildPoiRecord(record, routeCoordinates) {
    const normalizedRecord = normalizePoiRecord(record, record?.category);
    if (!normalizedRecord) return null;
    const distanceToRouteMeters = computeDistanceToRouteMeters(routeCoordinates, normalizedRecord.lat, normalizedRecord.lon);
    return {
        ...normalizedRecord,
        distanceToRouteMeters: Number.isFinite(distanceToRouteMeters) ? Math.round(distanceToRouteMeters) : null,
        dataStatus: normalizedRecord.dataStatus || "Source: OpenStreetMap"
    };
}
...
function renderEvStationList(stations) {
    ...
    stations.forEach((station) => {
        const li = document.createElement("li");
        const distanceLabel = Number.isFinite(Number(station.distanceToRouteMeters))
            ? `${Math.round(Number(station.distanceToRouteMeters))} m`
            : "N/A";
        li.textContent = `${station.name} | Distance to route: ${distanceLabel} | Availability: ${station.dataStatus}`;
        list.appendChild(li);
    });
}
...
function renderPoliceStationList(stations) {
    ...
    stations.forEach((station) => {
        ...
        const distanceLabel = Number.isFinite(Number(station.distanceToRouteMeters))
            ? `${Math.round(Number(station.distanceToRouteMeters))} m`
            : "N/A";
        li.textContent = `${station.name} | Distance to route: ${distanceLabel}`;
        list.appendChild(li);
    });
}
...
function drawPoiMarkersFromCurrentLists() {
    ...
    evStations = normalizePoiCollection(evStations, "ev");
    fuelStations = normalizePoiCollection(fuelStations, "fuel");
    policeStations = normalizePoiCollection(policeStations, "police");
    ...
    evStations.forEach((station) => {
        ...
        poiLayers.charging.addLayer(
            L.marker([station.lat, station.lon], { icon: makePoiIcon("EV", "#10b981") })
                .bindPopup(`${station.name}<br>Distance to route: ${distanceLabel}<br>Availability: ${station.dataStatus}`)
        );
    });
    ...
    policeStations.forEach((station) => {
        ...
        poiLayers.police.addLayer(
            L.marker([station.lat, station.lon], { icon: makePoiIcon("P", "#2563eb") })
                .bindPopup(`${station.name}<br>Distance to route: ${distanceLabel}`)
        );
    });
}
...
async function updatePoiOverlays(routeCoordinates) {
    const previousSnapshot = {
        ev: normalizePoiCollection(evStations, "ev"),
        fuel: normalizePoiCollection(fuelStations, "fuel"),
        police: normalizePoiCollection(policeStations, "police")
    };
    ...
    evStations = dedupePoiRecords(grouped.ev)
        ...
        .filter(Boolean)
        .filter((item) => Number.isFinite(item.distanceToRouteMeters))
...
function buildTollPlazaPlan(routeCoordinates, totalRouteDistanceKm, vehicle = "car", congestionLevel = "Medium") {
    ...
    const samplePoints = pickRouteSamplePoints(normalizedRoute, plazaCount);
    const congestionFactor = congestionLevel === "High" ? 1.12 : congestionLevel === "Medium" ? 1.06 : 1;
    const routeSeed = buildRouteSeed(normalizedRoute);
...
function smoothMove(marker, newLatLng, duration = 700) {
    ...
    const normalizedTarget = normalizeLatLngPair(newLatLng);
    if (!normalizedTarget || !start) return;
    const end = L.latLng(normalizedTarget);
...
function cacheOfflineRoute(payload) {
    try {
        const safePayload = sanitizeOfflineRouteCache(payload);
        if (!safePayload) return;
        localStorage.setItem(OFFLINE_ROUTE_CACHE_KEY, JSON.stringify(safePayload));
        offlineRouteCache = safePayload;
    } catch (error) {
        console.warn("Unable to store offline route cache:", error.message);
    }
}

function sanitizeOfflineRouteCache(cachePayload) {
    if (!cachePayload || typeof cachePayload !== "object") return null;
    const routes = (Array.isArray(cachePayload.routes) ? cachePayload.routes : [])
        .map((route) => {
            const safeGeometry = buildSafeRouteGeometry(route);
    ...
    const safeSource = normalizeLatLngPair(cachePayload.srcCoords);
    const safeDestination = normalizeLatLngPair(cachePayload.destCoords);
    const safeVia = (Array.isArray(cachePayload.viaCoords) ? cachePayload.viaCoords : [])
        .map((coords) => normalizeLatLngPair(coords))
        .filter(Boolean);

    return {
        ...cachePayload,
        routes,
        srcCoords: safeSource || null,
        destCoords: safeDestination || null,
        viaCoords: safeVia
    };
}
...
function getOfflineRoute(source, destination) {
    if (!offlineRouteCache) {
        ...
        if (raw) {
            try {
                offlineRouteCache = sanitizeOfflineRouteCache(JSON.parse(raw));
            } catch (error) {
...
async function renderRouteFromOfflineCache(cachePayload, requestedType = "default") {
    ...
    const durationMins = Math.max(1, Math.round(selectedRoute.duration / 60));
    const prediction = await getTrafficPrediction(hour, day, distanceKm, durationMins, vehicle, purpose, priority);
    const safeRouteGeometry = buildSafeRouteGeometry(selectedRoute);
    if (!safeRouteGeometry) {
        throw new Error("Cached route geometry is invalid. Please find the route again.");
    }
    const weatherSnapshot = await getRouteWeatherSnapshot(safeRouteGeometry.coordinates, hour);
    prediction.weather = weatherSnapshot.label;
    prediction.weather_source = weatherSnapshot.source;
    prediction.vehicle = vehicle;
    const nextSnapshot = {
        ...
        purpose,
        priority,
        routeCoordinates: safeRouteGeometry.coordinates,
        selectedVariantKey
    };
    lastRouteSnapshot = nextSnapshot;
    selectedRouteGeometry = safeRouteGeometry;
    selectedRouteCoordinates = safeRouteGeometry.coordinates.map(([lon, lat]) => [lat, lon]);
    selectedRouteKey = selectedVariantKey;

    let srcCoords = normalizeLatLngPair(sourceSelection?.coords || cachePayload.srcCoords || null);
    let destCoords = normalizeLatLngPair(destinationSelection?.coords || cachePayload.destCoords || null);
    let viaCoords = (Array.isArray(cachePayload.viaCoords) ? cachePayload.viaCoords : [])
        .map((coords) => normalizeLatLngPair(coords))
        .filter(Boolean);
    if (!srcCoords || !destCoords) {
        srcCoords = normalizeLatLngPair(await geocode(source, { referenceCoords: getMapCenterCoords() }));
        destCoords = normalizeLatLngPair(await geocode(destination, { referenceCoords: srcCoords || getMapCenterCoords() }));
    }
    if (!srcCoords || !destCoords) {
        throw new Error("Cached source/destination coordinates are invalid. Please enter locations again.");
    }
    if (!viaCoords.length && viaStops.length) {
        viaCoords = [];
        let viaReference = srcCoords;
        for (const stop of viaStops) {
            const coords = normalizeLatLngPair(await geocode(stop, { referenceCoords: viaReference }));
            if (!coords) continue;
            viaCoords.push(coords);
            viaReference = coords;
        }
    ...
    refreshTrafficHotspots(safeRouteGeometry.coordinates, prediction);
    await refreshEvStations(safeRouteGeometry.coordinates, prediction);
    refreshTollPlazas(safeRouteGeometry.coordinates, distanceKm, vehicle, prediction);
    setWorkspaceState("results");
    updateResultsUI({
        source,
        destination,
...
function loadCachedRealtimeData() {
    try {
        ...
        evStations = normalizePoiCollection(payload.evStations, "ev");
        fuelStations = normalizePoiCollection(payload.fuelStations, "fuel");
        policeStations = normalizePoiCollection(payload.policeStations, "police");
        tollPlazas = normalizeTollPlazaCollection(payload.tollPlazas);
        renderTrafficHotspotList(trafficHotspots);
        renderEvStationList(evStations);
        renderFuelStationList(fuelStations);
        renderPoliceStationList(policeStations);
